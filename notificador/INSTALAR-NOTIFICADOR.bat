...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$lnk = $s.CreateShortcut([IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup\Notificador Chamados Financeiros.lnk')); " ^
  "$lnk.TargetPath = 'wscript.exe'; " ^
  "$lnk.Arguments = '\"%~dp0notificador-oculto.vbs\"'; " ^
  "$lnk.WorkingDirectory = '%~dp0'; " ^
  "$lnk.Description = 'Notificador de Chamados Financeiros (avisos na barra de tarefas)'; " ^
  "$lnk.Save()"
